...
url :: URL
url = "http://semanticsarchive.net/cgi-bin/browse.pl"
...
composeTwete ident = do
  (_, info) <- curlGetString ("http://semanticsarchive.net/Archive/"++
                              ident ++"/info.txt") []
...
process ident info = let authorTitle = unwords . words $ -- rmv double-spaces
                                       getAuthors info ++
                                       ": "++ getTitle info in
                     let link = " http://semanticsarchive.net/Archive/"++ ident in
