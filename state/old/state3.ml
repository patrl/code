...
let surface_scope verb = pwfa ar_in (pwfa ar_out r0);;
let inverse_scope verb = pwfa ar_out (pwfa ar_in r0);;
...
[fun s1 ->
  [fun s2 ->
    apply (apply (unit (fun p q -> p & q)) [s1]) [s2]  
  ]
];;
...
lower (pwfa (pwfa every p0) (pwfa r0 (narrowish (pwfa some (pwfa r0 (he 0))))));; 
lower (pwfa (pwfa every p0) (pwfa r0 (pwfa some (pwfa r0 (he 0)))));; 
